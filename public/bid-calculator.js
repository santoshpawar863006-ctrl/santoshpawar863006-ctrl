...
(() => {
  ...
  function ensurePanel(tender) {
    ...
    panel.innerHTML = `
      <div class="bc-head">
        <div>
          ...
          <p>Estimate site cost, break-even bid, safe discount vs ECV, and target profit. Starts from a standard profile for the tender category; edit the percentages to match your own rates.</p>
        </div>
        <span class="bc-badge">Contractor tool</span>
      </div>
      <div class="bc-actions">
        <button type="button" class="bc-btn primary" id="bcCalculateBtn">Calculate Bid</button>
        <button type="button" class="bc-btn secondary" id="bcRecalcBtn" hidden>Recalculate from edits</button>
      </div>
      <div class="bc-status" id="bcStatus">${amount ? 'Click Calculate Bid to generate a planning estimate for this tender.' : 'Tender value is missing. Enter ECV manually after opening the calculator defaults.'}</div>
    ...
    document.getElementById('bcCalculateBtn')?.addEventListener('click', () => runCalculate(tender));
    document.getElementById('bcRecalcBtn')?.addEventListener('click', () => runCalculate(tender, { from_edits: true }));
    return panel;
  }
  ...
  function tenderPayload(tender, options = {}) {
    ...
    if (options.from_edits) body.assumptions = readEditedAssumptions();
    return body;
  }
  ...
  function setBusy(busy, label) {
    ...
    if (btn) {
      btn.disabled = busy;
      btn.textContent = busy ? (label || 'Calculating…') : 'Calculate Bid';
    }
  ...
  async function runCalculate(tender, options = {}) {
    ...
    try {
      const response = await fetch('/api/bid_calculator', {
        method: 'POST',
  ...
  function renderResult(payload, tender) {
    ...
    status.textContent = payload.message || 'Bid plan ready.';
