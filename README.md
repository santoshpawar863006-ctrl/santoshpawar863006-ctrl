...
An action every 4 hours (`.github/workflows/collect-results.yml`) runs `collect_results.py`, which stores awarded tenders — winner, every bidder's quoted total and rank from KPPP's public comparative statement — in `public/results-lite.json` (cached in `data/results-cache.json`). It also stores every bidder's item-wise quoted rates (`data/item-rates-cache.json`) and builds `public/rates-lite.json`: for each BOQ item (schedule code + unit + description), what winning (L1) bidders quoted against the department's rate. The tender page shows these under each BOQ item. The bid % is computed against the tender's estimated value, because the % in KPPP's statement double-counts the estimate.
